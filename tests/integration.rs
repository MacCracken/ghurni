...
fn test_gasoline_engine() {
    let mut engine = Engine::new(EngineType::Gasoline, 4);
    let samples = engine.synthesize(3000.0, 0.5, 44100.0, 1.0).unwrap();
...
#[test]
fn test_two_stroke() {
    let mut engine = Engine::new(EngineType::TwoStroke, 1);
    let samples = engine.synthesize(5000.0, 0.8, 44100.0, 0.5).unwrap();
    assert!(!samples.is_empty());
    assert!(samples.iter().all(|s| s.is_finite()));
}
...
fn test_all_engine_types() {
    ...
    for t in &types {
        let mut engine = Engine::new(*t, 4);
        let result = engine.synthesize(2000.0, 0.5, 44100.0, 0.3);
...
fn test_firing_frequency() {
    let engine = Engine::new(EngineType::Gasoline, 4);
...
fn test_gear_all_materials() {
    ...
    for m in &materials {
        let mut gear = Gear::new(32, *m);
        let result = gear.synthesize(1500.0, 44100.0, 0.3);
...
fn test_gear_mesh_frequency() {
    let gear = Gear::new(32, GearMaterial::Steel);
...
fn test_motor_all_types() {
    ...
    for t in &types {
        let mut motor = Motor::new(*t, 4);
        let result = motor.synthesize(3000.0, 0.5, 44100.0, 0.3);
        assert!(result.is_ok(), "failed for {:?}", t);
        assert!(result.unwrap().iter().all(|s| s.is_finite()));
    }
}
...
fn test_turbine() {
    ...
    let samples = turbine.synthesize(10000.0, 44100.0, 0.5).unwrap();
    assert!(!samples.is_empty());
    assert!(samples.iter().all(|s| s.is_finite()));
}
...
fn test_turbine_open() {
    let mut turbine = Turbine::new(3, 0.0); // Open propeller, no duct
    let samples = turbine.synthesize(2000.0, 44100.0, 0.5).unwrap();
    assert!(!samples.is_empty());
    assert!(samples.iter().all(|s| s.is_finite()));
}
...
fn test_clock_all_types() {
    ...
    for t in &types {
        ...
        let result = clock.synthesize(44100.0, 1.0);
        assert!(result.is_ok(), "failed for {:?}", t);
        assert!(result.unwrap().iter().all(|s| s.is_finite()));
    }
}
...
fn test_higher_load_more_energy() {
    let mut lo = Engine::new(EngineType::Gasoline, 4);
    let mut hi = Engine::new(EngineType::Gasoline, 4);
    let lo_samples = lo.synthesize(3000.0, 0.2, 44100.0, 1.0).unwrap();
    let hi_samples = hi.synthesize(3000.0, 0.9, 44100.0, 1.0).unwrap();
